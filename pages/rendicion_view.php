...
// Parámetros
$id_rms = $_GET['seleccionar'] ?? null;
$cliente_data = null;
$remesa_data = null;
$conceptos_cliente = [];
$conceptos_agencia = [];

// Cargar datos solo si hay ID y en contexto web
if ($id_rms && php_sapi_name() !== 'cli') {
    try {
        $pdo = getDBConnection();
        if ($pdo) {
            ...
            if ($remesa_data) {
                ...
                $stmt = $pdo->prepare("SELECT * FROM rendicion WHERE id_rms = ? AND tipo_concepto = 'cliente' ORDER BY id_rndcn");
                $stmt->execute([$id_rms]);
                $conceptos_cliente = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Cargar conceptos agencia
                $stmt = $pdo->prepare("SELECT * FROM rendicion WHERE id_rms = ? AND tipo_concepto = 'agencia' ORDER BY id_rndcn");
                $stmt->execute([$id_rms]);
...
<div class="container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.2rem;">
        ...
        <a href="/pages/rendicion_listas.php" class="btn-secondary" style="text-decoration: none; padding: 0.4rem 0.8rem;">
                ...
                <table class="data-table">
                    ...
                    <tbody id="lista-conceptos-cliente">
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                <td><?= htmlspecialchars($c['item_rndcn'] ?? '') ?></td>
                                <td><?= htmlspecialchars($c['concepto_rndcn'] ?? '') ?></td>
                                <td><?= number_format($c['monto_pago_rndcn'] ?? 0, 0, ',', '.') ?></td>
                                <td>
                                    <a href="#" class="btn-edit" title="Editar" onclick="editarRendicion(<?= $c['id_rndcn'] ?>, 'cliente')">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="#" class="btn-delete" title="Eliminar" onclick="eliminarRendicion(<?= $c['id_rndcn'] ?>)">
                    ...
                    <tbody id="lista-conceptos-agencia">
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="#" class="btn-edit" title="Editar" onclick="editarRendicion(<?= $c['id_rndcn'] ?>, 'agencia')">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="#" class="btn-delete" title="Eliminar" onclick="eliminarRendicion(<?= $c['id_rndcn'] ?>)">
                                        <i class="fas fa-trash-alt"></i>
...
<div id="submodal-rendicion" class="submodal">
    <div class="submodal-content" style="max-width: 600px; padding: 1.8rem; position: relative;">
        ...
        <form id="form-rendicion">
            ...
            <input type="hidden" id="id_rndcn">
            <input type="hidden" id="tipo_concepto">

            <div style="margin-bottom: 1.2rem;">
                <label style="display: block; margin-bottom: 0.4rem;">Ítem:</label>
                <input type="number" id="item_rndcn" style="width: 6ch; height: 2.3rem; padding: 0.3rem;">
            </div>
...
function abrirSubmodalRendicion(tipo) {
    ...
    // Limpiar formulario
    document.getElementById('id_rndcn').value = '';
    document.getElementById('item_rndcn').value = '';
...
function guardarRendicion() {
    ...
    formData.append('action', document.getElementById('id_rndcn').value ? 'actualizar_rendicion' : 'crear_rendicion');
    ...
    formData.append('item_rndcn', document.getElementById('item_rndcn').value);
    ...
    if (document.getElementById('id_rndcn').value) {
        formData.append('id_rndcn', document.getElementById('id_rndcn').value);
    }
...
function editarRendicion(id, tipo) {
    alert('Edición no implementada en esta versión.');
}
